...
lpTag.agentSDK = (function () {
    ...
    /**
     * Gets a given key from the agent data.
     * @param data The key to get. For example 'visitorInfo.visitorName'
     * @param cbSuccess The success callback
     * @param cbError The error callback
     */
    function setConsumerProfile(data, cbSuccess, cbError) {
        var successCallback = _safelyWrapCallback(cbSuccess, '\'setConsumerProfile\' success');
        var errorCallback = _safelyWrapCallback(cbError, '\'setConsumerProfile\' error', true);
        if (channel && data) {
            _info('Calling "setConsumerProfile');
            try {
                channel.request({
                    appName: appNames.manager,
                    reqName: reqNames.setConsumerProfile,
                    data: { json: data }
                }, function (err) {
                    return err ? errorCallback(err) : successCallback();
                });
            } catch (error) {
                _error('Error calling "setConsumerProfile". The error: ' + error);
                errorCallback(error);
            }
        } else {
            var errorMessage;
            if (!channel) {
                errorMessage = 'No channel defined when calling "setConsumerProfile". Maybe you are missing a call to "init"?';
            } else if (!data) {
                errorMessage = 'No data provided when calling "setConsumerProfile"';
            }
            if (errorMessage) {
                _error(errorMessage);
                errorCallback(errorMessage);
